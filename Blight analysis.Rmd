...
crime %>% select(CATEGORY) %>% 
  group_by(CATEGORY) %>% 
  summarize(cnt=n()) %>% 
  arrange(cnt) %>% View()
adr_loc <- crime %>% #slice(1:10) %>%
  filter(CATEGORY == "BURGLARY") %>%
  select(ADDRESS, LAT, LON) %>%
  rename(addr = ADDRESS, lat = LAT, lng = LON) %>%
  bind_rows(adr_loc)
...
```{r}
demol <-parse_addr(demolitions, "site_location") %>% 
  distinct(addr) %>%
  mutate(blighted = TRUE)

non_blighted <- adr_loc %>%
  distinct(addr) %>%
  anti_join(demol, by = "addr") %>%
  mutate(blighted = FALSE)

data_set <- non_blighted %>%
  sample_n(nrow(demol), replace = FALSE) %>% 
  bind_rows(demol)

data_set <- violations %>% 
  parse_addr("ViolationAddress") %>% 
  inner_join(data_set, by="addr") %>% 
  group_by(addr) %>% summarise(cnt = n()) %>% 
  right_join(data_set, by="addr") %>% 
